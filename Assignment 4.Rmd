...
title: | 
  ...
  | 2022-2023
author: |
  | Melle Stephen Oudshoorn
  | m.s.oudshoorn@students.uu.nl
  | http://www.github.com/msoud7
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW$treat <- ifelse(GrowthSW$revolutions > 0, 
                         "More than 0 rev", "0 rev")
Sum_GrowthSW <- datasummary(growth + rgdp60 ~ treat*(Mean + median + sd + min + max),
                            data = GrowthSW)
print(Sum_GrowthSW)
...
In these tables you can see that the mean growth for countries that did not have a revolution is significantly higher compared to the countries that did have revolutions. The median is also higher for countries that did not have revolutions, however, that difference is a bit smaller than the prior mentioned.
...
```{r}
t_test_growth <- t.test(growth ~ treat,
                        data = GrowthSW)
print(t_test_growth)
```
...
The p-value of the test is 0.06871. This means that for a chance of about 0.07 there is no true difference between growth in between both of the groups. 
...
the variable 'rgdp60' is the value of GDP per capita in 1960, converted into 1960 US dollars. The other variables all are measures of growth. We use rgdp60 as a baseline to indicate how the growth has continued over time.
...
```{r}
 model_total <- list(model1, model2, model3, model4) |>
...
The 4th variable. When the R^2 value is high, it indicates that a large percentage of the variability in the dependent variable can be explained by the independent variables. In the 4th column the value is the highest. which means that the main driver of economic growth predicted by this model is: education.
...
```{r}
library(kableExtra)
model_format <- list(model1, model2, model3, model4) |>
  modelsummary(stars=T, 
               gof_map = c("nobs", "r.squared")) 

model_format <- model_format |>  column_spec(2, color = "white", background = "red")
